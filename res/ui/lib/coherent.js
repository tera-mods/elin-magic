	...
	*/
	...
	engine._OnWindowLoaded = function () {
		...
		if (engine._BindingsReady) {
			...
			{
				engine.trigger('alert', message);
			}
			
			if(typeof window.init_window == "function"){
				engine.on('init_window', window.init_window);
			}
			
			if(typeof window.update_coin == "function"){
				engine.on('update_coin', window.update_coin);
			}
			
			if(typeof window.purchase_result == "function") {
				engine.on('purchase_result', window.purchase_result);
			}
		}
	};
	...
	function RegisterCallback(callback) {
		...
		if (typeof callback == 'function') {
			id = new Date().valueOf().toString(36) + Math.floor((Math.random()*72)).toString(36);
			engine.on(id, function callonce() {
				callback.apply(null,arguments);
				engine.off(id, callonce);
			});
		}
		return id;
	}

	_tera_client_proxy_.query_coin = function (callback) {
		var callbackId = RegisterCallback(callback)
		_tera_client_proxy_.query_coin_internal(callbackId);
	};
	
	_tera_client_proxy_.purchase = function () {
		var callbackId = RegisterCallback(arguments[arguments.length-1]);
		var args = [...arguments];
		args.splice(args.length-1,1,callbackId);
		_tera_client_proxy_.purchase_internal(args);
	};
